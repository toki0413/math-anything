...
from schemas import MathSchema, GoverningEquation, BoundaryCondition
...
class MathDiff:
    ...
    def compare(cls, schema1: MathSchema, schema2: MathSchema) -> DiffResult:
        ...
        eq_comparison = cls._compare_equations(
            schema1.mathematical_model.governing_equations if schema1.mathematical_model else [],
            schema2.mathematical_model.governing_equations if schema2.mathematical_model else [],
        )
        ...
        bc_comparison = cls._compare_boundary_conditions(
            schema1.mathematical_model.boundary_conditions if schema1.mathematical_model else [],
            schema2.mathematical_model.boundary_conditions if schema2.mathematical_model else [],
        )
    ...
    @classmethod
    def _equation_similarity(cls, eq1: GoverningEquation, eq2: GoverningEquation) -> float:
    ...
    def _compare_boundary_conditions(
        ...
        return {
            ...
            'unique_first_types': list(set(bc1_by_type.keys()) - set(bc2_by_type.keys())),
            'unique_second_types': list(set(bc2_by_type.keys()) - set(bc1_by_type.keys())),
        }
    ...
    def _suggest_parameter_mappings(
        ...
        for p1 in params1:
            for p2 in params2:
                similarity = difflib.SequenceMatcher(None, p1.lower(), p2.lower()).ratio()
                if similarity > 0.8 and (p1, p2) not in [(m['first'], m['second']) for m in mappings]:
                    mappings.append({
                        'first': p1,
                        'second': p2,
    ...
    def _calculate_similarity(
        ...
        total_eqs = len(eq_comparison['common']) + len(eq_comparison['different']) + \
                   len(eq_comparison['unique_first']) + len(eq_comparison['unique_second'])
        if total_eqs > 0:
        ...
        if bc_comparison:
            ...
            total_bc_types = common_bc_types + len(bc_comparison.get('unique_first_types', [])) + \
        ...
        if schema1.numerical_method and schema2.numerical_method:
            ...
            nm_score = 0.0
            if nm1.discretization.space_discretization is not None and \
               nm1.discretization.space_discretization == nm2.discretization.space_discretization:
                nm_score += 0.5
            if nm1.discretization.time_integrator is not None and \
               nm1.discretization.time_integrator == nm2.discretization.time_integrator:
                nm_score += 0.5
    ...
    def _generate_summary(
        ...
        if similarity_pct > 80:
            ...
            parts.append(f"Moderate similarity ({similarity_pct:.0f}%): These models have some common elements but differ in important ways.")
        else:
            parts.append(f"Low similarity ({similarity_pct:.0f}%): These models represent fundamentally different mathematical approaches.")
        
        # Add specifics
        if result.common_equations:
            parts.append(f"They share {len(result.common_equations)} governing equations.")
        
        if result.parameter_mapping:
            high_conf = [m for m in result.parameter_mapping if m.get('confidence') == 'high']
            if high_conf:
                parts.append(f"Found {len(high_conf)} clear parameter mappings between engines.")
