...
class Visualizer:
    ...
    def _to_mermaid(self, schema: Dict[str, Any]) -> str:
        ...
        if approxs and self.config.show_approximations:
            lines.append(
                f'    Root --> ApproxRoot["Approximation Hierarchy\n({len(approxs)} levels)"]'
            )
        ...
        if constraints and self.config.show_constraints:
            ...
            lines.append(
                f'    ConstRoot --> ConstStatus["{satisfied}/{len(constraints)} Satisfied"]'
            )
    ...
    def _to_graphviz(self, schema: Dict[str, Any]) -> str:
        ...
        if approxs and self.config.show_approximations:
            lines.append(
                f'    approx [label="Approximations ({len(approxs)})", fillcolor="#e8f5e9"];'
            )
