        ...
        for eq in getattr(schema, 'governing_equations', []):
            ...
            fingerprint = MathFingerprint(
                ...
                metadata={
                    ...
                    "variable_types": [v.get("type", "unknown") 
                                      for v in eq.variables],
                    "tensor_ranks": [v.get("tensor_rank", 0) 
                                    for v in eq.variables 
                                    if "tensor_rank" in v],
                }
            )
        ...
        for bc in getattr(schema, 'boundary_conditions', []):
            fingerprint = MathFingerprint(
                ...
                metadata={
                    ...
                    "has_tensor": any(v.get("type") == "tensor" 
                                     for v in bc.variables),
            ...
            if hasattr(obj, 'tensor_rank') and obj.tensor_rank is not None:
                fingerprint = MathFingerprint(
                    ...
                    metadata={
                        ...
                        "has_symmetry": "symmetric" in str(obj.properties).lower() if hasattr(obj, 'properties') else False,
                    }
                )
                self.fingerprints.append(fingerprint)
