...
class ExpressionSimplifier:
    ...
    def _simplify_mul(self, node: Node) -> Node:
        ...
        if (left.node_type == NodeType.CONST and 
            right.node_type == NodeType.MUL and
            right.left.node_type == NodeType.CONST):
            new_const = left.value * right.left.value
            return Node(NodeType.MUL, 
    ...
    def _simplify_div(self, node: Node) -> Node:
        ...
        # (x * const) / const = x
        if (left.node_type == NodeType.MUL and
        ...
        # (x * const) / (y * const) = x / y
        if (left.node_type == NodeType.MUL and right.node_type == NodeType.MUL and
            left.left.node_type == NodeType.CONST and right.left.node_type == NodeType.CONST):
    ...
    def _is_zero(self, node: Node) -> bool:
        ...
        return (node.node_type == NodeType.CONST and 
                node.value is not None and 
                abs(node.value) < 1e-10)
    
    def _is_one(self, node: Node) -> bool:
        """Check if node is one."""
        return (node.node_type == NodeType.CONST and 
                node.value is not None and 
                abs(node.value - 1.0) < 1e-10)
    ...
    def _trees_equal(self, n1: Node, n2: Node) -> bool:
        ...
        return (self._trees_equal(n1.left, n2.left) and 
                self._trees_equal(n1.right, n2.right))
