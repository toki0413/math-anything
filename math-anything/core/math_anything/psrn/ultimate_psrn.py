...
class AdaptiveConstantOptimizer:
    ...
    def optimize(
        ...
        for iteration in range(self.max_iter):
            # 当前表达式
            current_expr = self._substitute_constants(
                expr_template, placeholders, constants
            )
        ...
        # 返回最优结果
        final_expr = self._substitute_constants(
            expr_template, placeholders, best_constants
        )
...
class UltimatePSRN(PSRN):
    ...
    def _create_random_tree(
        self, variable_names: List[str], max_depth: int = 4
    ) -> Node:
        """创建随机树."""
        import random
