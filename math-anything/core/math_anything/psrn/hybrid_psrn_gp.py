...
class HybridPSRNGP:
    ...
    def fit(
        ...
        # 获取 PSRN 候选
        self.psrn_candidates = self._extract_psrn_candidates(
            psrn_sr, X, y, variable_names
        )
        ...
        print("=" * 60)
        print(
            f"  PSRN Phase:  {psrn_time:.2f}s, best MSE: {self.psrn_candidates[0][1]:.6f}"
        )
    ...
    def _create_random_tree(
        self, variable_names: List[str], max_depth: int = 3
    ) -> Node:
        """创建随机表达式树."""
        import random
