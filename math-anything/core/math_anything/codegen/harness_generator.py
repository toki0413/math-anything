...
class HarnessAutoGenerator:
    ...
    def _generate_with_jinja(
        ...
        confidence_metrics = [
            {'name': 'Command Extraction', 'value': confidence.get('command_extraction', 0)},
            {'name': 'Parameter Extraction', 'value': confidence.get('parameter_extraction', 0)},
            {'name': 'Equation Mapping', 'value': confidence.get('equation_mapping', 0)},
    ...
    def _create_review_checklist(
        ...
        for eq in mappings.get('equations', [])[:3]:
            checklist.append(
                f"[ ] Validate equation: {eq.get('name', 'Unknown')}"
            )
