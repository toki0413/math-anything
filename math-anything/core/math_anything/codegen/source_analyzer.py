...
class SourceCodeAnalyzer:
    ...
    COMMAND_PATTERNS = {
        "cpp": {
            ...
            "variable_decl": re.compile(
                r"(\w+)\s+(\w+)\s*=\s*([^;]+);\s*//\s*(.+)", re.IGNORECASE
            ),
        },
        "python": {
            "class_command": re.compile(
                r"class\s+(\w+)\s*\([^)]*(?:Command|Parser|Handler)", re.IGNORECASE
            ),
            "arg_parsing": re.compile(
                r"self\.(\w+)\s*=\s*(?:args|argv|params)\[(\d+)\]", re.IGNORECASE
            ),
            ...
            "argparse": re.compile(
                r'add_argument\s*\(\s*["\']([^"\']+)["\']', re.IGNORECASE
            ),
        },
        "java": {
            "class_command": re.compile(
            ...
            "method_param": re.compile(
                r"public\s+\w+\s+(\w+)\s*\([^)]*String\s+(\w+)", re.IGNORECASE
            ),
        },
    }
    ...
    def _calculate_coverage(
        ...
        command_coverage = min(
            1.0, commands_found / 20
        )  # Assume ~20 major commands typical
        param_coverage = min(
            1.0, parameters_found / 50
        )  # Assume ~50 parameters typical
    ...
    def _analyze_file(
        ...
        if "class_command" in patterns:
            for match in patterns["class_command"].finditer(content):
                ...
                is_command = any(
                    hint.lower() in class_name.lower() for hint in entry_hints
                )
...
def quick_analyze(source_dir: str) -> Dict[str, Any]:
    ...
    print(f"\n📊 Coverage Metrics:")
    print(
        f"   Files: {coverage.get('files_successfully_processed', 0)}/{coverage.get('total_files_scanned', 0)}"
    )
    print(
