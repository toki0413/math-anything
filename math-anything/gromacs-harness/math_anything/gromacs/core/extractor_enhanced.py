...
class EnhancedGromacsExtractor(GromacsExtractor):
    ...
    def _parse_gro_file(self, gro_path: str):
        ...
        if len(lines) > num_atoms + 2:
            ...
            if len(box_parts) >= 3:
                self.simulation_summary.box_dimensions = tuple(
                    float(x) for x in box_parts[:3]
                )
    ...
    def _generate_simulation_summary(self) -> MDSimulationSummary:
        ...
        if "tcoupl" in self.mdp_params:
            ...
            if tcoupl != "no":
                summary.ensemble = (
                    "NVT"
                    if "pcoupl" not in self.mdp_params
                    or self.mdp_params.get("pcoupl") == "no"
                    else "NPT"
                )
            else:
    ...
    def _validate_simulation(self):
        ...
        if self.simulation_summary.timestep > 0.002:
            self.validation_warnings.append(
                f"时间步长 {self.simulation_summary.timestep} ps 可能过大，建议 <= 0.002 ps"
            )
...
def generate_gromacs_summary_report(enhanced_result: Dict[str, Any]) -> str:
    ...
    if summary.box_dimensions[0] > 0:
        report.append(
            f"  盒子尺寸: {summary.box_dimensions[0]:.2f} x {summary.box_dimensions[1]:.2f} x {summary.box_dimensions[2]:.2f} nm"
        )
    report.append("")
