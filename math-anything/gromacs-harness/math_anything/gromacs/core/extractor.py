...
class GromacsExtractor:
    ...
    def extract_thermodynamics(
        ...
        if len(pressure) > 1:
            ...
            if pressure_var > 0:
                kappa_T = volume_mean * pressure_var / (np.mean(temperature) * 1.380649e-23)
    ...
    def extract_energies(
        ...
        return {
            ...
            'energy_fluctuation': float(np.std(total_energy) / np.mean(total_energy)) if np.mean(total_energy) != 0 else 0,
        }
