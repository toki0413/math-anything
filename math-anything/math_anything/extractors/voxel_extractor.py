...
class VoxelMathExtractor:
    ...
    Example:
        ...
        logger.info(math_structure['discretization'])

        # Get boundary conditions
        logger.info(math_structure['boundary_conditions'])
        ```
    """
    ...
    def extract(
        ...
        **kwargs,
        ...
        result["grid_info"] = self._extract_grid_info(voxel_data, physical_origin, voxel_size)

        # Extract scale mapping
        result["scale_mapping"] = self._extract_scale_mapping(voxel_data.shape, physical_origin, voxel_size)

        # Extract discretization semantics
        result["discretization"] = self._extract_discretization_semantics(voxel_data, simulation_type)
        ...
        result["interpolation_rules"] = self._extract_interpolation_rules(voxel_data, simulation_type)

        # Extract mathematical semantics
        result["mathematical_semantics"] = self._extract_mathematical_semantics(voxel_data, simulation_type)
    ...
    def _extract_boundary_conditions(
        ...
        for name, idx in face_boundaries:
            boundary = self._create_boundary_description(name, idx, simulation_type)
            boundaries.append(boundary)

        # Internal boundaries (if mask provided)
        if boundary_mask is not None:
            internal = self._extract_internal_boundaries(boundary_mask, simulation_type)
        ...
        return [
            {
                ...
                "numerical_scheme": (
                    BoundaryScheme.CURVED_BOUNDARY.value
                    if simulation_type == "lattice_boltzmann"
                    else "immersed_boundary"
                ),
