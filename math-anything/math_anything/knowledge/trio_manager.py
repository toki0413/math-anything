...
class KnowledgeTrioManager:
    ...
    Example:
        ...
        logger.info(trio.get_transparency_report())
    ...
    def query(self, schema, max_results: int = 5) -> List[KnowledgeResult]:
        ...
        if KnowledgeSource.ARXIV in self._enabled_sources and not self.cache.is_offline_mode:
            arxiv_results = self._query_arxiv(schema, max_results)
            results.extend(arxiv_results)

        if KnowledgeSource.WIKIDATA in self._enabled_sources and not self.cache.is_offline_mode:
            wikidata_results = self._query_wikidata(schema, max_results)
            results.extend(wikidata_results)

        if KnowledgeSource.NIST_POTENTIALS in self._enabled_sources and not self.cache.is_offline_mode:
            ...
            for paper in papers:
                results.append(
                    KnowledgeResult(
                        source="arXiv",
                        title=paper.title,
                        content=(paper.abstract[:500] + "..." if len(paper.abstract) > 500 else paper.abstract),
    ...
    def _log_query(self, schema, fingerprints: List[Any]):
        ...
        entry = {
            ...
            "num_fingerprints": (len(fingerprints) if isinstance(fingerprints, list) else 1),
            "fingerprint_types": (
                [fp.fingerprint_type.value for fp in fingerprints] if isinstance(fingerprints, list) else ["dict"]
            ),
    ...
    def get_settings(self) -> Dict[str, Any]:
        ...
        return {
            "trio_enabled": self._trio_enabled,
            "enabled_sources": [s.value for s in self._enabled_sources],
            "disabled_sources": [s.value for s in KnowledgeSource if s not in self._enabled_sources],
            "offline_mode": self.cache.is_offline_mode,
