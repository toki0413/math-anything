...
class MathFingerprintExtractor:
        ...
        for eq in getattr(schema, "governing_equations", []):
            ...
            fingerprint = MathFingerprint(
                ...
                metadata={
                    ...
                    "tensor_ranks": [v.get("tensor_rank", 0) for v in eq.variables if "tensor_rank" in v],
                },
            )
            ...
            if hasattr(obj, "tensor_rank") and obj.tensor_rank is not None:
                fingerprint = MathFingerprint(
                    ...
                    metadata={
                        ...
                        "num_components": (len(obj.components) if hasattr(obj, "components") and obj.components else 0),
                        "has_symmetry": (
                            "symmetric" in str(obj.properties).lower() if hasattr(obj, "properties") else False
                        ),
                    },
                )
                self.fingerprints.append(fingerprint)
    ...
    def _fingerprint_coupling(self, schema):
        ...
        for graph in getattr(schema, "computational_graphs", []):
            ...
            fingerprint = MathFingerprint(
                ...
                structure_hash=self._hash_structure(json.dumps(coupling_pattern, sort_keys=True)),
                metadata={
    ...
    def _extract_coupling_pattern(self, graph) -> Dict[str, Any]:
        ...
        return {
            ...
            "edge_connectivity": [{"from": edge.from_node, "to": edge.to_node} for edge in graph.edges],
        }
