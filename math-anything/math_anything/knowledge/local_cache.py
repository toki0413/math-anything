...
from math_anything.utils.serialization import safe_load, signed_dumps
...
class LocalKnowledgeCache:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        total_size = sum(Path(m["file"]).stat().st_size for m in self.metadata.values() if Path(m["file"]).exists())
    ...
    def _enforce_size_limit(self):
        ...
        current_size = sum(Path(m["file"]).stat().st_size for m in self.metadata.values() if Path(m["file"]).exists())
