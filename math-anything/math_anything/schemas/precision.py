...
def _filter_none(data: dict) -> dict:
    ...
    return {k: v for k, v in data.items() if v is not None and not (isinstance(v, (dict, list)) and len(v) == 0)}
...
class EnhancedMathSchema:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.precision_metadata:
            result["precision_metadata"] = {k: v.to_dict() for k, v in self.precision_metadata.items()}
        return result
