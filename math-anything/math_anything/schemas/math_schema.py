...
class BoundaryCondition:
    ...
    software_implementation: Dict[str, object]
    dual_role: Optional[Dict[str, object]] = None
...
class ComputationalNode:
    ...
    id: str
    type: str
    math_semantics: Dict[str, object]
...
class ComputationalGraph:
    ...
    execution_topology: Dict[str, object] = field(default_factory=dict)
...
class NumericalMethod:
    ...
    parallelization: Dict[str, object] = field(default_factory=dict)
...
class MathematicalModel:
    ...
    initial_conditions: List[Dict[str, object]] = field(default_factory=list)
    constitutive_relations: List[Dict[str, object]] = field(default_factory=list)
    coupling_conditions: List[Dict[str, object]] = field(default_factory=list)
...
class MetaInfo:
    ...
    material_context_declaration: Dict[str, object] = field(default_factory=dict)
...
class MathSchema:
    ...
    raw_symbols: Dict[str, object] = field(default_factory=dict)
    symbolic_constraints: List[SymbolicConstraint] = field(default_factory=list)
    ...
    def load(cls, path: str) -> "MathSchema":
        """Load MathSchema from JSON file.
...
class StreamingSchemaBuilder:
    """流式 Schema 构建器 — 减少内存峰值.

    逐字段构建 MathSchema，避免同时持有所有中间对象。
    适用于大规模提取（如百万级自由度的 FEM 模型）。
    """

    def __init__(self):
        self._schema = MathSchema()
        self._field_count = 0

    def add_equations(self, equations: List[GoverningEquation]) -> StreamingSchemaBuilder:
        self._schema.mathematical_model.governing_equations.extend(equations)
        self._field_count += len(equations)
        return self

    def add_boundary_conditions(self, bcs: List[BoundaryCondition]) -> StreamingSchemaBuilder:
        self._schema.mathematical_model.boundary_conditions.extend(bcs)
        self._field_count += len(bcs)
        return self

    def add_conservation(self, props: Dict[str, ConservationProperty]) -> StreamingSchemaBuilder:
        self._schema.conservation_properties.update(props)
        self._field_count += len(props)
        return self
...
class MathSchemaPool:
    """轻量级 MathSchema 对象池，减少 GC 压力."""

    _pool: list[MathSchema] = []
    _max_size: int = 32

    @classmethod
    def acquire(cls) -> MathSchema:
        """获取一个 MathSchema 实例（从池或新建）."""
        if cls._pool:
            return cls._pool.pop()
        return MathSchema()

    @classmethod
    def release(cls, schema: MathSchema) -> None:
        """归还 MathSchema 实例到池中."""
        if len(cls._pool) < cls._max_size:
            schema.mathematical_model.governing_equations = []
            schema.mathematical_model.boundary_conditions = []
            schema.mathematical_model.constitutive_relations = []
            schema.numerical_method = NumericalMethod()
            schema.computational_graph = ComputationalGraph()
            schema.conservation_properties = {}
            schema.raw_symbols = {}
            schema.symbolic_constraints = []
            cls._pool.append(schema)
