...
logger.info(f"EML v2 backend: {'Rust' if _accel.using_rust else 'Python'}")
...
class ImprovedSymbolicRegression:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, variable_names: Optional[List[str]] = None) -> Node:
        ...
        for gen in range(self.generations):
            ...
            if best_fitness < 1e-12:
                logger.info(f"Gen {gen}: Perfect fit found!")
                break

            if no_improvement_count > 20:
                logger.info(f"Gen {gen}: No improvement for 20 generations, stopping")
                break
        ...
        if best_tree is None:
            ...
            return self.builder.var(self.variables[0]) if self.variables else self.builder.const(0.0)
    ...
    def _init_population(self) -> List[Node]:
        ...
        for i in range(self.population_size):
            if i < self.population_size // 2:
                tree = self._random_tree_full(min(i % self.max_depth + 1, self.max_depth))
            else:
                tree = self._random_tree_grow(min(i % self.max_depth + 1, self.max_depth))
    ...
    def _fitness(self, tree: Node, X: np.ndarray, y: np.ndarray) -> float:
        ...
        try:
            ...
            for i in range(len(X)):
                vars_dict = {name: float(X[i, j]) for j, name in enumerate(self.variables)}
    ...
    def _tournament_select(self, population: List[Node], fitness: List[float], tournament_size: int = 5) -> Node:
    ...
    def _mutate(self, tree: Node) -> Node:
        ...
        if mutation_type < 0.3:
            ...
            nodes = tree.get_all_nodes()
            const_nodes = [(n, p, d) for n, p, d in nodes if n.node_type == NodeType.CONST]
            if const_nodes:
...
def discover_equation(X: np.ndarray, y: np.ndarray, variable_names: Optional[List[str]] = None, **kwargs) -> str:
    """Discover equation from data."""
...
def eml_closure(base: list[float], max_depth: int = 5, max_size: int = 100000) -> list[float]:
    """Compute EML closure over a set of base values.

    Iteratively applies eml(a, b) = exp(a) - ln(b) to all pairs
    until no new values are produced or max_depth is reached.

    Uses Rust acceleration when available, falls back to pure Python.
    """
    return _accel.eml_closure(base, max_depth, max_size)
