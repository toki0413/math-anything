...
def _collect_free(term: Term, acc: set[str]) -> None:
    ...
    if kind == TermKind.VAR:
        ...
        _collect_free(term.domain, acc)
        bound = {term.var_name}
        child_free = free_vars(term.codomain) - bound
        acc.update(child_free)
    elif kind == TermKind.LAMBDA:
        bound = {term.var_name}
        child_free = free_vars(term.body) - bound
        acc.update(child_free)
    elif kind == TermKind.APP:
        _collect_free(term.func, acc)
        _collect_free(term.arg, acc)
    elif kind == TermKind.SIGMA:
        _collect_free(term.fst_type, acc)
        bound = {term.var_name}
        child_free = free_vars(term.snd_type) - bound
        acc.update(child_free)
    elif kind == TermKind.PAIR:
        _collect_free(term.fst, acc)
        _collect_free(term.snd, acc)
    elif kind in (TermKind.PROJ1, TermKind.PROJ2):
        _collect_free(term.pair, acc)
    elif kind == TermKind.IDENTITY:
        _collect_free(term.typ, acc)
        _collect_free(term.lhs, acc)
        _collect_free(term.rhs, acc)
    elif kind == TermKind.REFL:
        _collect_free(term.typ, acc)
        _collect_free(term.term, acc)
    elif kind == TermKind.SYM:
        _collect_free(term.typ, acc)
        _collect_free(term.proof, acc)
    elif kind == TermKind.TRANS:
        _collect_free(term.typ, acc)
        _collect_free(term.p, acc)
        _collect_free(term.q, acc)
    elif kind == TermKind.CONG:
        _collect_free(term.func, acc)
        _collect_free(term.proof, acc)
    elif kind == TermKind.TRANSPORT:
        _collect_free(term.motive, acc)
        _collect_free(term.eq_proof, acc)
        _collect_free(term.value, acc)
...
def _subst(term: Term, var_name: str, repl: Term, bound: set[str]) -> Term:
    ...
    if kind == TermKind.VAR:
        return repl if term.name == var_name and term.name not in bound else term

    elif kind == TermKind.UNIVERSE:
        return term

    elif kind == TermKind.PI:
        new_bound = bound | {term.var_name}
        new_dom = _subst(term.domain, var_name, repl, bound)
        new_cod = _subst(term.codomain, var_name, repl, new_bound)
        if term.var_name == var_name:
            # 变量被遮蔽，不替换 codomain 中的同名变量
            return Pi(term.var_name, new_dom, term.codomain)
        return Pi(term.var_name, new_dom, new_cod)

    elif kind == TermKind.LAMBDA:
        new_bound = bound | {term.var_name}
        new_body = _subst(term.body, var_name, repl, new_bound)
        if term.var_name == var_name:
            return Lam(term.var_name, term.body)
        return Lam(term.var_name, new_body)

    elif kind == TermKind.APP:
        return App(_subst(term.func, var_name, repl, bound), _subst(term.arg, var_name, repl, bound))

    elif kind == TermKind.SIGMA:
        new_bound = bound | {term.var_name}
        new_fst = _subst(term.fst_type, var_name, repl, bound)
        new_snd = _subst(term.snd_type, var_name, repl, new_bound)
        if term.var_name == var_name:
            return Sigma(term.var_name, new_fst, term.snd_type)
        return Sigma(term.var_name, new_fst, new_snd)

    elif kind == TermKind.PAIR:
        return Pair(_subst(term.fst, var_name, repl, bound), _subst(term.snd, var_name, repl, bound))

    elif kind == TermKind.PROJ1:
        return Proj1(_subst(term.pair, var_name, repl, bound))
    elif kind == TermKind.PROJ2:
        return Proj2(_subst(term.pair, var_name, repl, bound))

    elif kind == TermKind.IDENTITY:
        return Identity(
            _subst(term.typ, var_name, repl, bound),
            _subst(term.lhs, var_name, repl, bound),
            _subst(term.rhs, var_name, repl, bound),
        )

    elif kind == TermKind.REFL:
        return Refl(_subst(term.typ, var_name, repl, bound), _subst(term.term, var_name, repl, bound))
        ...
        return Sym(
            ...
            _subst(term.lhs, var_name, repl, bound),
            _subst(term.rhs, var_name, repl, bound),
            _subst(term.proof, var_name, repl, bound),
        )

    elif kind == TermKind.TRANSPORT:
        return Transport(
            _subst(term.motive, var_name, repl, bound),
            _subst(term.eq_proof, var_name, repl, bound),
            _subst(term.source_type, var_name, repl, bound),
            _subst(term.a, var_name, repl, bound),
            _subst(term.b, var_name, repl, bound),
            _subst(term.value, var_name, repl, bound),
        )

    elif kind == TermKind.ANNOTATION:
        return Annotation(_subst(term.term, var_name, repl, bound), _subst(term.typ, var_name, repl, bound))

    elif kind == TermKind.INDUCTIVE:
        return term

    elif kind == TermKind.CONSTRUCT:
        return Construct(term.type_name, term.ctor_name, tuple(_subst(a, var_name, repl, bound) for a in term.args))

    elif kind == TermKind.IND_ELIM:
        return IndElim(
            term.type_name,
            _subst(term.motive, var_name, repl, bound),
            tuple(_subst(m, var_name, repl, bound) for m in term.methods),
            _subst(term.target, var_name, repl, bound),
        )
...
def whnf(term: Term) -> Term:
    ...
    if term.kind == TermKind.APP:
        ...
        if func.kind == TermKind.LAMBDA:
            # β-归约
            return whnf(substitute(func.body, func.var_name, term.arg))
...
def term_to_str(term: Term, _prec: int = 0) -> str:
    ...
    if kind == TermKind.VAR:
        ...
        return f"Type_{term.level}" if term.level > 0 else "Type"
    elif kind == TermKind.PI:
        fv = free_vars(term.codomain)
        if term.var_name not in fv:
            # 非依赖: A → B
            s = f"{term_to_str(term.domain, 2)} → {term_to_str(term.codomain, 1)}"
        else:
            s = f"Π({term.var_name}:{term_to_str(term.domain)}). {term_to_str(term.codomain)}"
        return f"({s})" if _prec > 1 else s
    elif kind == TermKind.LAMBDA:
        return f"λ{term.var_name}. {term_to_str(term.body)}"
    elif kind == TermKind.APP:
        return f"{term_to_str(term.func, 2)}({term_to_str(term.arg, 2)})"
    elif kind == TermKind.SIGMA:
        fv = free_vars(term.snd_type)
        if term.var_name not in fv:
            s = f"{term_to_str(term.fst_type, 2)} × {term_to_str(term.snd_type, 1)}"
        else:
            s = f"Σ({term.var_name}:{term_to_str(term.fst_type)}). {term_to_str(term.snd_type)}"
        return f"({s})" if _prec > 1 else s
    elif kind == TermKind.PAIR:
        return f"({term_to_str(term.fst)}, {term_to_str(term.snd)})"
    elif kind == TermKind.PROJ1:
        return f"π₁({term_to_str(term.pair)})"
    elif kind == TermKind.PROJ2:
        return f"π₂({term_to_str(term.pair)})"
    elif kind == TermKind.IDENTITY:
        return f"Id_{{{term_to_str(term.typ)}}}({term_to_str(term.lhs)}, {term_to_str(term.rhs)})"
    elif kind == TermKind.REFL:
        return f"refl_{{{term_to_str(term.typ)}}}({term_to_str(term.term)})"
    elif kind == TermKind.SYM:
        return f"sym({term_to_str(term.proof)})"
    elif kind == TermKind.TRANS:
        return f"trans({term_to_str(term.p)}, {term_to_str(term.q)})"
    elif kind == TermKind.CONG:
        return f"cong({term_to_str(term.func)}, {term_to_str(term.proof)})"
    elif kind == TermKind.TRANSPORT:
        return f"transport({term_to_str(term.motive)}, {term_to_str(term.eq_proof)}, {term_to_str(term.value)})"
    elif kind == TermKind.ANNOTATION:
        return f"({term_to_str(term.term)} : {term_to_str(term.typ)})"
    elif kind == TermKind.INDUCTIVE:
        return term.name
    elif kind == TermKind.CONSTRUCT:
        args = ", ".join(term_to_str(a) for a in term.args)
        return f"{term.ctor_name}" + (f"({args})" if args else "")
    elif kind == TermKind.IND_ELIM:
        return f"ind_{term.type_name}(...)"
