...
class LayerPruner:
    ...
    def _top_k_prune(self, scores: np.ndarray, layer_idx: int) -> List[int]:
        """简单 Top-K 剪枝."""
        keep_ratio = max(self.config.min_keep_ratio, self.config.initial_keep_ratio * (0.7**layer_idx))
...
class EarlyStopController:
    ...
    def check_should_stop(self, layer_idx: int, current_best_score: float) -> Tuple[bool, str]:
        """检查是否应该早停.
