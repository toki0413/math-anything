...
class RandomTokenGenerator(TokenGenerator):
    ...
    def _evaluate_expressions(self, expressions: List[str], X: np.ndarray, variable_names: List[str]) -> np.ndarray:
        """向量化评估表达式."""
...
class GPTokenGenerator(TokenGenerator):
    ...
    def _extract_subtrees(self, sr: ImprovedSymbolicRegression, X: np.ndarray, variable_names: List[str]) -> List[Node]:
        """从 GP 种群中提取优质子树."""
        # 简化实现：返回最佳树的子树
    ...
    def _evaluate_tokens(self, tokens: List[Node], X: np.ndarray, variable_names: List[str]) -> np.ndarray:
        """评估所有 token 的数值."""
        n_samples = X.shape[0]
        n_tokens = len(tokens)
...
class MCTSTokenGenerator(TokenGenerator):
    ...
    def _generate_candidates(self, X: np.ndarray, variable_names: List[str]) -> List[Tuple[str, np.ndarray]]:
        """生成候选表达式."""
        X.shape[0]
        candidates = []
