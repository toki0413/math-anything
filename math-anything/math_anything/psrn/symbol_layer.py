...
class SymbolLayer:
    ...
    def deduce_expression(
        ...
        Returns:
            推导出的表达式字符串
        """
        return self._recursive_deduce(len(offset_tensors) - 1, best_idx, offset_tensors, base_exprs)
    ...
    def _recursive_deduce(
        ...
        if right_idx == -1:
            # 一元算子
            left_expr = self._recursive_deduce(layer_idx - 1, left_idx, offset_tensors, base_exprs)
            return self._format_unary(op, left_expr)
        else:
            # 二元算子
            left_expr = self._recursive_deduce(layer_idx - 1, left_idx, offset_tensors, base_exprs)
            right_expr = self._recursive_deduce(layer_idx - 1, right_idx, offset_tensors, base_exprs)
    ...
    def memory_estimate(self, n_inputs: int, n_layers: int) -> Dict[str, int]:
        ...
        for layer in range(1, n_layers + 1):
            ...
            next_dim = n_u * current + n_bs * current * current + n_bt * current * (current + 1) // 2
            dims[layer] = next_dim
            current = next_dim
