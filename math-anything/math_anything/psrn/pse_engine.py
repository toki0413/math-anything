...
class PSEEngine:
    ...
    def discover(
        ...
        for iteration in range(self.config.max_iterations):
            if verbose:
                logger.info(f"\n{'=' * 50}")
                logger.info(f"PSE Iteration {iteration + 1}/{self.config.max_iterations}")
                logger.info(f"{'=' * 50}")
            ...
            if verbose:
                logger.info(f"PSRN found: {best_expr_iter}")
                logger.info(f"MSE: {best_mse_iter:.6e}")
                logger.info(f"Search space: {self.psrn.get_search_space_size()}")
            ...
            if best_mse < self.config.early_stop_mse:
                if verbose:
                    logger.info(f"Early stop: MSE {best_mse:.2e} < threshold")
                break

            if no_improvement_count >= self.config.no_improvement_limit:
                if verbose:
                    logger.info(f"Early stop: No improvement for {no_improvement_count} iterations")
                break
