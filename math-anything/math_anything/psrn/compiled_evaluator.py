...
class CompiledEvaluator:
    ...
    def _ast_to_func(self, node: ast.AST, variable_names: List[str]) -> Callable:
        ...
        if isinstance(node, ast.Name):
            ...
            if isinstance(node.op, ast.Add):
                ...
                return lambda **kwargs: np.power(np.abs(left_func(**kwargs)), right_func(**kwargs))
    ...
    def evaluate_batch_vec(
        ...
        优先使用 Rust 加速路径，不可用时回退到纯 Python 向量化。
        ...
        # Rust 加速路径
        if _accel.using_rust:
            try:
                data_flat = X.ravel().tolist()
                rust_results = _accel.batch_eval_expressions(expressions, variable_names, n_samples, data_flat)
                if rust_results and len(rust_results) == n_exprs:
                    results = np.zeros((n_samples, n_exprs))
                    for i, col in enumerate(rust_results):
                        if col and len(col) == n_samples:
                            results[:, i] = col
                        else:
                            results[:, i] = np.nan
                    logger.debug("evaluate_batch_vec: Rust backend used (%d exprs)", n_exprs)
                    return results
                # Rust 返回空结果，降级到 Python
                logger.debug("evaluate_batch_vec: Rust returned empty, falling back to Python")
            except (ValueError, TypeError, RuntimeError):
                logger.debug("evaluate_batch_vec: Rust failed, falling back to Python")
        ...
        logger.debug("evaluate_batch_vec: Python backend used (%d exprs)", n_exprs)
        return results
...
def benchmark_evaluators():
    ...
    logger.info("=" * 60)
    logger.info("Evaluator Performance Benchmark")
    logger.info("=" * 60)
    ...
    for i, expr in enumerate(expressions):
        for j in range(n_samples):
            ...
            try:
                # NOTE: safe_eval cannot handle numpy vectorized function calls
                # (e.g., sin(ndarray)) because it doesn't support top-level
                # function calls with array arguments. Keeping restricted eval
                # with __builtins__ disabled as the safest feasible option.
                # Input is validated via validate_eval_expr() before eval().
                from ..utils.safe_eval import validate_eval_expr

                validate_eval_expr(expr)
                baseline_results[j, i] = eval(expr, {"__builtins__": {}}, var_dict)
            except (ValueError, TypeError, ZeroDivisionError, OverflowError):
                baseline_results[j, i] = np.nan
    baseline_time = time.time() - t0
    logger.info(f"Time: {baseline_time:.4f}s")
    ...
    logger.info(f"Time: {compiled_time:.4f}s")
    logger.info(f"Speedup vs baseline: {baseline_time / compiled_time:.1f}x")
    ...
    t0 = time.time()
    fast_layer.compute_layer(offsets, X)
    fast_time = time.time() - t0
    logger.info(f"Time: {fast_time:.4f}s")
    logger.info(f"Speedup vs baseline: {baseline_time / fast_time:.1f}x")
    ...
    logger.info(f"Second run (cached): {cached_time:.4f}s")
    logger.info(f"Cache speedup: {compiled_time / cached_time:.1f}x")

    logger.info("\n" + "=" * 60)
    logger.info("Summary")
    logger.info("=" * 60)
    logger.info(f"{'Method':<30} {'Time (ms)':>10} {'Speedup':>10}")
    logger.info("-" * 52)
    logger.info(f"{'Python eval (baseline)':<30} {baseline_time * 1000:>10.1f} {'1.0x':>10}")
    logger.info(f"{'CompiledEvaluator (1st)':<30} {compiled_time * 1000:>10.1f} {baseline_time / compiled_time:>9.1f}x")
    logger.info(f"{'CompiledEvaluator (cached)':<30} {cached_time * 1000:>10.1f} {baseline_time / cached_time:>9.1f}x")
    logger.info(f"{'FastSymbolLayer':<30} {fast_time * 1000:>10.1f} {baseline_time / fast_time:>9.1f}x")
