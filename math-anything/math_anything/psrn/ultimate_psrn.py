...
class AdaptiveConstantOptimizer:
    ...
    def optimize(
        ...
        for iteration in range(self.max_iter):
            ...
            # 计算 MSE 和梯度
            try:
                mse, gradients = self._compute_gradient(current_expr, X, y, variable_names, constants, placeholders)
                ...
                # 遇到错误，回退到最佳值
                logger.debug(f"Adam optimization step failed: {e}")
    ...
    def _substitute_constants(self, template: str, placeholders: List[str], values: np.ndarray) -> str:
        """将常量值代入模板."""
        result = template
    ...
    def _evaluate(self, expr: str, X: np.ndarray, y: np.ndarray, variable_names: List[str]) -> float:
        """评估表达式 MSE."""
        try:
            local_vars = {
                "np": np,
...
class UltimatePSRN(PSRN):
    ...
    def fit(
        ...
        logger.info("=" * 60)
        logger.info("Ultimate PSRN - 综合优化")
        logger.info("=" * 60)

        # Phase 1: 深度 PSRN 探索
        logger.info("Phase 1: 深度 PSRN 探索 (4层)")
        logger.info("-" * 50)

        base_exprs, base_values = self._build_base_expressions(X, variable_names, token_exprs, token_values)
        ...
        for layer_idx in range(self.ultimate_config.n_layers):
            ...
            # 前向传播
            new_exprs, new_values, _ = layer.forward(current_exprs, current_values, layer_idx)
            ...
            logger.info(
                f"  Layer {layer_idx}: {len(new_exprs)} → {len(pruned_exprs)} (best MSE: {mses[sorted_indices[0]]:.4f})"
            )
        ...
        if self.ultimate_config.use_adaptive_constants:
            ...
            if optimized_candidates:
                ...
                logger.info(f"  最优常量优化后 MSE: {best_cand['mse']:.6f}")
            else:
        ...
        if self.ultimate_config.use_gp_refinement:
            logger.info("Phase 3: GP 精细优化")
            logger.info("-" * 50)

            best_expr, best_mse = self._gp_refinement(best_cand["expr"], X, y, variable_names)

            logger.info(f"  GP 优化后 MSE: {best_mse:.6f}")
        else:
        ...
        logger.info("=" * 60)
        logger.info(f"最终最优表达式: {best_expr}")
        logger.info(f"最终 MSE: {best_mse:.6f}")
        logger.info("=" * 60)
    ...
    def _gp_refinement(
        ...
        if best_tree:
            ...
            if best_mse < initial_mse:
                logger.info(f"  GP 发现更好的解 (MSE: {best_mse:.6f} vs {initial_mse:.6f})")
                return best_expr, best_mse
            else:
                logger.info(f"  PSRN 结果更好，保留 (MSE: {initial_mse:.6f})")
...
class UltimatePSRNSymbolicRegression:
    ...
    def fit(
        ...
        """拟合数据."""
        config = UltimatePSRNConfig(n_layers=self.n_layers, use_gp_refinement=self.use_gp_refinement, **self.kwargs)
