...
class PSRN:
    ...
    def __init__(self, config: Optional[PSRNConfig] = None):
        ...
        self.layers: List[SymbolLayer] = [SymbolLayer(self.config.symbol_config) for _ in range(self.config.n_layers)]
    ...
    def fit(
        ...
        for layer_idx, layer in enumerate(self.layers):
            current_exprs, current_values, offsets = layer.forward(current_exprs, current_values, layer_idx)
        ...
        # 反向推导最优表达式
        best_expr = self.layers[-1].deduce_expression(best_idx, self._offset_tensors, self._base_exprs)

        # 获取 top-k
        top_k_indices = np.argsort(all_mses)[:10]
        top_k = [
            (
                self.layers[-1].deduce_expression(idx, self._offset_tensors, self._base_exprs),
                all_mses[idx],
            )
