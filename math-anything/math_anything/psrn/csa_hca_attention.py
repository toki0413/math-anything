...
class HCALayer:
    ...
    def _deep_evaluate(self, expr: str, values: np.ndarray, y: np.ndarray, X: np.ndarray) -> float:
        """深度评估（高计算成本，更精确）."""
        scores = {}
        ...
        if "linear_fit" in self.depth_metrics:
            try:
                ...
                logger.debug(f"Linear fit score computation failed: {e}")
                scores["linear_fit"] = 0
        ...
        if "nonlinear_potential" in self.depth_metrics:
            try:
                ...
                logger.debug(f"Nonlinear potential score computation failed: {e}")
                scores["nonlinear_potential"] = 0
        ...
        if "eml_compatibility" in self.depth_metrics:
            if "eml" in expr or "exp" in expr or "log" in expr:
                ...
                try:
                    ...
                    logger.debug(f"EML compatibility score computation failed: {e}")
                    scores["eml_compatibility"] = 0.3
            else:
...
class CSAHCAAttentionGenerator(TokenGenerator):
    ...
    def generate(
        ...
        if self.adaptive_schedule:
            ...
            csa_ratio, _hca_ratio = self.csa_ratio, self.hca_ratio
        ...
        if reward_history and len(hca_output) < self.n_tokens:
            top_historical = sorted(reward_history.items(), key=lambda x: x[1], reverse=True)[
            ...
            for token, _ in top_historical:
                if token not in [h[0] for h in hca_output]:
                    ...
                    try:
                        ...
                        logger.debug(f"Failed to evaluate historical token '{token}': {e}")
    ...
    def _fallback_generate(self, X: np.ndarray, variable_names: List[str]) -> Tuple[List[str], np.ndarray]:
        """回退生成器."""
        candidates = []
