...
warnings.warn("enhanced_psrn is deprecated, use ultimate_psrn instead", DeprecationWarning, stacklevel=2)
...
class EnhancedPSRNConfig(PSRNConfig):
    ...
    def __post_init__(self):
        if self.pruning_schedule is None:
            ...
            self.pruning_schedule = [max(0.05, 0.5 * (0.6**i)) for i in range(self.n_layers)]
...
class AdaptiveConstantOptimizer:
    ...
    def optimize(
        self, expr_template: str, X: np.ndarray, y: np.ndarray, variable_names: List[str]
    ) -> Tuple[str, float]:
        """优化表达式中的常量.
            ...
            try:
                ...
                for p in placeholders:
                    ...
                    try:
                        ...
                        logger.debug(f"Gradient computation failed for {p}: {e}")
                        grads[p] = 0
    ...
    def _evaluate_expr(self, expr: str, X: np.ndarray, y: np.ndarray, variable_names: List[str]) -> np.ndarray:
        """评估表达式."""
        # 构建局部变量
        local_vars = {
            "np": np,
...
class EnhancedPSRN(PSRN):
    ...
    def fit(
        ...
        for layer_idx, layer in enumerate(self.layers):
            # 前向传播
            new_exprs, new_values, offsets = layer.forward(current_exprs, current_values, layer_idx)

            # 应用剪枝
            keep_ratio = self.enhanced_config.pruning_schedule[layer_idx]
            pruned_exprs, pruned_values, prune_stats = self._prune_layer(new_exprs, new_values, y, keep_ratio)

            logger.info(
                f"  Layer {layer_idx}: {len(new_exprs)} → {len(pruned_exprs)} ({prune_stats['prune_ratio']:.1%} pruned)"
            )
        ...
        if self.constant_optimizer and "{" in best_expr:
            best_expr, best_mse = self.constant_optimizer.optimize(best_expr, X, y, variable_names or ["x"])
    ...
    def _select_best_from_all_layers(
        ...
        for layer_idx, (exprs, values) in enumerate(zip(self._layer_outputs, self._layer_values)):
            mses = np.mean((values - y.reshape(-1, 1)) ** 2, axis=0)
