...
class AdaptiveAttentionGenerator(TokenGenerator):
    ...
    def generate(
        ...
        # 3. 生成大规模候选池
        candidates = self._generate_enhanced_pool(X, y, variable_names, problem_features)

        # 4. 计算自适应分数
        scored_candidates = []
        for expr, values, features in candidates:
            score = self._compute_adaptive_score(expr, values, y, X, features, problem_features)
        ...
        if remaining and n_explore > 0:
            ...
            indices = np.random.choice(len(remaining), size=n_sample, replace=False, p=probs)
            explore_candidates = [remaining[i] for i in indices]
        else:
            explore_candidates = []

        # 6. 多样性筛选
        selected = self._diversity_filter(exploit_candidates + explore_candidates, self.n_tokens)
    ...
    def _analyze_problem(self, X: np.ndarray, y: np.ndarray, variable_names: List[str]) -> Dict:
        """分析问题特征，指导候选生成."""
        features = {
    ...
    def _generate_enhanced_pool(
        ...
        for i, name in enumerate(variable_names):
            ...
            if np.all(x_vals < 5):  # 避免exp爆炸
                ...
                candidates.append((f"exp({name})", exp_vals, {"type": "exp", "complexity": 2}))
            if np.all(x_vals > 0):
                candidates.append((f"log({name})", np.log(x_vals + 1e-10), {"type": "log", "complexity": 2}))
            ...
            for i in range(min(3, n_vars)):
                for j in range(min(3, n_vars)):
                    if i != j:
                        ...
                        # EML核心
                        eml_vals = np.exp(np.clip(X[:, i], -50, 50)) - np.log(np.abs(X[:, j]) + 1e-10)
                        candidates.append(
                            (f"eml({name_i},{name_j})", eml_vals, {"type": "eml", "complexity": 4, "eml_core": True})
                        )

                        # EML变体
                        candidates.extend(
                            [
                                (f"(exp({name_i})-log({name_j}))", eml_vals, {"type": "eml_expanded", "complexity": 5}),
                            ]
                        )
        ...
        if n_vars >= 2:
            ...
            for i in range(min(3, n_vars)):
                for j in range(i + 1, min(3, n_vars)):
                    name_i, name_j = variable_names[i], variable_names[j]
                    prod_vals = X[:, i] * X[:, j]
                    candidates.append((f"({name_i}*{name_j})", prod_vals, {"type": "product", "complexity": 2}))
    ...
    def _compute_adaptive_score(
        ...
        try:
            ...
            logger.debug(f"Linearity score computation failed: {e}")
            scores["linearity"] = 0
        ...
        if features.get("eml_core") or "eml" in expr:
            ...
            try:
                ...
                logger.debug(f"EML potential score computation failed: {e}")
    ...
    def _update_weights_from_history(self, reward_history: Dict[str, float]):
        ...
        # 分析成功的token特征
        successful_tokens = [(token, reward) for token, reward in reward_history.items() if reward > 0.7]
...
class HybridAttentionGenerator(AdaptiveAttentionGenerator):
    ...
    def __init__(self, n_tokens: int = 20, strategy_weights: Optional[Dict[str, float]] = None, **kwargs):
    ...
    def generate(
        ...
        if n_by_strategy["attention"] > 0:
            ...
            tokens, values = super().generate(X, y, variable_names, current_tokens, reward_history)
        ...
        if n_by_strategy["evolutionary"] > 0 and reward_history:
            top_tokens = sorted(reward_history.items(), key=lambda x: x[1], reverse=True)[
        ...
        if n_by_strategy["gradient"] > 0:
            self._generate_gradient_tokens(X, y, variable_names, n_by_strategy["gradient"])
            # 这里简化处理，实际应该计算值
        ...
        for token in final_tokens:
            try:
                ...
                logger.debug(f"Token evaluation failed for '{token}': {e}")
    ...
    def _generate_gradient_tokens(self, X: np.ndarray, y: np.ndarray, variable_names: List[str], n: int) -> List[str]:
        """基于梯度方向生成token."""
        # 简化实现
        tokens = []
