...
if HAS_NUMBA:
    ...
    @njit(cache=True, fastmath=True)
    def _apply_operator_numba(op_code: int, x: np.ndarray, y: np.ndarray, out: np.ndarray):
        """Numba 加速的算子应用.
...
class OptimizedEvaluator:
        ...
        try:
            ...
            for j, expr in enumerate(expressions):
                try:
                    ...
                    logger.debug(f"Evaluation failed for expression: {e}")
                    values[:, j] = np.nan
    ...
    def _evaluate_expr(self, expr: str, X: np.ndarray, variable_names: List[str]) -> np.ndarray:
        """评估单个表达式."""
        # 检查缓存
    ...
    def _compile_expr(self, expr: str, variable_names: List[str]) -> Callable:
        ...
        # NOTE: safe_eval cannot replace this eval because func_code is a
        # dynamically generated lambda string (e.g., "lambda X, names: ...")
        # that requires full Python evaluation. The __builtins__ restriction
        # and safe_dict whitelist prevent arbitrary code execution.
...
class IncrementalEvaluator:
    ...
    def register_layer_output(self, layer_idx: int, expressions: List[str], values: np.ndarray):
        """注册某层的输出结果."""
