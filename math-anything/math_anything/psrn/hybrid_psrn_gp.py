...
class HybridPSRNGP:
    ...
    def fit(
        ...
        """
        logger.info("=" * 60)
        logger.info("Hybrid PSRN-GP Search")
        logger.info("=" * 60)

        # Phase 1: PSRN 快速探索
        logger.info("Phase 1: PSRN Exploration")
        logger.info("-" * 40)
        ...
        logger.info(f"  PSRN found {len(self.psrn_candidates)} candidates")
        logger.info(f"  Best PSRN MSE: {self.psrn_candidates[0][1]:.6f}")
        logger.info(f"  Time: {psrn_time:.2f}s")

        # Phase 2: GP 精细优化
        logger.info("Phase 2: GP Refinement")
        logger.info("-" * 40)
        ...
        logger.info(f"  Best GP MSE: {self.best_fitness:.6f}")
        logger.info(f"  Time: {gp_time:.2f}s")

        # 总结果
        logger.info("=" * 60)
        logger.info("Summary")
        logger.info("=" * 60)
        logger.info(f"  PSRN Phase:  {psrn_time:.2f}s, best MSE: {self.psrn_candidates[0][1]:.6f}")
        logger.info(f"  GP Phase:    {gp_time:.2f}s, best MSE: {self.best_fitness:.6f}")
        logger.info(f"  Total:       {psrn_time + gp_time:.2f}s")
        logger.info(f"  Improvement: {(self.psrn_candidates[0][1] - self.best_fitness):.6f}")
    ...
    def _extract_psrn_candidates(
        ...
        if best_expr:
            try:
                ...
                logger.debug(f"Failed to evaluate PSRN best expression: {e}")
        ...
        for token_gen in ["fast", "random"]:
            try:
                ...
                logger.debug(f"Failed to evaluate PSRN expression with {token_gen}: {e}")
    ...
    def _build_initial_population(
        ...
        for expr, mse in candidates[:20]:  # 取前20个
            try:
                ...
                logger.debug(f"Failed to parse expression to tree: {e}")
        ...
        while len(population) < self.config.gp_population_size:
            if candidates:
                ...
                try:
                    ...
                    logger.debug(f"Failed to parse/mutate expression: {e}")
            else:
                # 完全随机
                tree = self._create_random_tree(variable_names or ["x"], max_depth=3)
                population.append(tree)
