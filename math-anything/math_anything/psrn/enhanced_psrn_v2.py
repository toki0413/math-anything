...
warnings.warn("enhanced_psrn_v2 is deprecated, use ultimate_psrn instead", DeprecationWarning, stacklevel=2)
...
class ConservativeEnhancedPSRN(PSRN):
    ...
    def fit(
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            logger.info(f"  Layer {layer_idx}: {len(new_exprs)} → {len(pruned_exprs)}")
