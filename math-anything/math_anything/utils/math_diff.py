...
Example:
    ...
    logger.info(f"Boundary conditions changed: {len(report.bc_changes)}")
...
class DiffReport:
    ...
    def print_summary(self):
        ...
        logger.info("=" * 70)
        logger.info("MATH ANYTHING DIFF REPORT")
        logger.info("=" * 70)

        if self.old_source and self.new_source:
            logger.info("Comparing:")
            logger.info(f"  Old: {self.old_source}")
            logger.info(f"  New: {self.new_source}")

        summary = self.to_dict()["summary"]
        logger.info(f"Total Changes: {summary['total_changes']}")
        logger.info(f"  Critical: {summary['critical']}")
        logger.info(f"  Structural: {summary['structural']}")
        logger.info(f"  Numerical: {summary['numerical']}")
        logger.info(f"  Boundary: {summary['boundary']}")
        logger.info(f"  Conservation: {summary['conservation']}")
        logger.info(f"  Computational: {summary['computational']}")
        logger.info(f"  Parameters: {summary['parameters']}")

        if self.critical_changes:
            logger.info("-" * 70)
            logger.info("CRITICAL CHANGES:")
            logger.info("-" * 70)
            for change in self.critical_changes:
                logger.info(f"[{change.type.name}] {change.path}")
...
class MathDiffer:
    ...
    Example:
        ...
        if report.has_critical_changes:
            logger.warning("Warning: Critical mathematical changes detected!")
        ```
    """
    ...
    def compare(
        ...
        old_data = old_schema.to_dict() if hasattr(old_schema, "to_dict") else old_schema
        new_data = new_schema.to_dict() if hasattr(new_schema, "to_dict") else new_schema
    ...
    def _compare_boundary_conditions(self, old_data: Dict, new_data: Dict):
        ...
        for bc_id in new_ids - old_ids:
            ...
            tensor_info = f" (rank-{mo.get('tensor_rank')} tensor)" if mo.get("tensor_rank") else ""
        ...
        for bc_id in old_ids & new_ids:
            ...
            old_comps = {tuple(c.get("index", [])): c.get("value") for c in old_mo.get("components", [])}
            new_comps = {tuple(c.get("index", [])): c.get("value") for c in new_mo.get("components", [])}
    ...
    def _compare_computational_graph(self, old_data: Dict, new_data: Dict):
        ...
        for node_id in set(old_nodes.keys()) & set(new_nodes.keys()):
            old_mode = old_nodes[node_id].get("math_semantics", {}).get("updates", {}).get("mode")
            new_mode = new_nodes[node_id].get("math_semantics", {}).get("updates", {}).get("mode")
            ...
            if old_mode != new_mode:
                self.report.computational_changes.append(
                    Change(
                        ...
                        severity=(
                            "critical" if "implicit" in str(old_mode) or "implicit" in str(new_mode) else "warning"
                        ),
                    )
                )
