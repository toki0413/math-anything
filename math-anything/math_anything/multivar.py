...
from .eml_v2 import ExprBuilder, ImprovedSymbolicRegression, Node
...
class MultiVariableDiscovery:
    ...
    def discover(
        ...
        *arrays: np.ndarray,
        ...
        logger.info(f"Variable correlations with target: {correlations}")
    ...
    def _create_templates(self, correlations: Dict[int, float]) -> List[Node]:
        ...
        important_vars = [i for i, corr in correlations.items() if corr > self.correlation_threshold]
    ...
    def _init_with_templates(self, sr: ImprovedSymbolicRegression, templates: List[Node], n_vars: int) -> List[Node]:
        """Initialize population with templates."""
    ...
    def _evolve_with_templates(
        ...
        for gen in range(sr.generations):
            ...
            if best_fitness < 1e-12:
                logger.info(f"Gen {gen}: Perfect fit!")
                break

            if no_improvement_count > 25:
                logger.info(f"Gen {gen}: No improvement, stopping")
                break
            ...
            if gen % 10 == 0:
                logger.info(f"Gen {gen}: Best fitness = {best_fitness:.6f}")
...
def analyze_interactions(X: np.ndarray, y: np.ndarray, variable_names: Optional[List[str]] = None) -> Dict[str, Any]:
