...
class SourceCodeAnalyzer:
    ...
    COMMAND_PATTERNS = {
        "cpp": {
            ...
            "arg_parsing": re.compile(r"(?:arg|args)\[(\d+)\]\s*[=)]", re.IGNORECASE),
            "force_cast": re.compile(r'force->\w+\s*\(\s*["\']([^"\']+)["\']\s*\)', re.IGNORECASE),
            "error_message": re.compile(
                r'error->\w+\([^)]*["\']([^"\']+(?: illegal| expected| must be))',
                re.IGNORECASE,
            ),
            "variable_decl": re.compile(r"(\w+)\s+(\w+)\s*=\s*([^;]+);\s*//\s*(.+)", re.IGNORECASE),
        },
        "python": {
            "class_command": re.compile(r"class\s+(\w+)\s*\([^)]*(?:Command|Parser|Handler)", re.IGNORECASE),
            "arg_parsing": re.compile(r"self\.(\w+)\s*=\s*(?:args|argv|params)\[(\d+)\]", re.IGNORECASE),
            "click_command": re.compile(
                r"@click\.(?:command|option|argument)[^\n]*\n+def\s+(\w+)",
                re.DOTALL | re.IGNORECASE,
            ),
            "argparse": re.compile(r'add_argument\s*\(\s*["\']([^"\']+)["\']', re.IGNORECASE),
        },
        "java": {
            "class_command": re.compile(
            ...
            "method_param": re.compile(r"public\s+\w+\s+(\w+)\s*\([^)]*String\s+(\w+)", re.IGNORECASE),
        },
    }
    ...
    def analyze(
        ...
        logger.info(f"   Analyzing {len(source_files)} files...")
        ...
        for file_path in source_files:
            try:
                file_cmds, file_params, file_eqs = self._analyze_file(file_path, entry_hints)
        ...
        # Get file extensions
        extensions = list(set(f.suffix for f in source_files if f.suffix))
        ...
        if coverage_metrics["estimated_command_coverage"] < 0.3:
            self._warnings.append(
                "LOW COMMAND COVERAGE: Found few commands. Source may use patterns not covered by this analyzer."
            )
    ...
    def _analyze_file(
        ...
        if "class_command" in patterns:
            for match in patterns["class_command"].finditer(content):
                ...
                is_command = any(hint.lower() in class_name.lower() for hint in entry_hints)
    ...
    def _extract_equations_from_comments(
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            has_math_keyword = any(
                keyword in line.lower() for keywords in self.MATH_KEYWORDS.values() for keyword in keywords
            )
    ...
    def infer_constraints(
        ...
        constraint_patterns = [
            (
                rf"if\s*\(\s*{param_name}\s*([<>=]+)\s*([\d.]+)",
                lambda m: f"{param_name} {m.group(1)} {m.group(2)}",
            ),
            (
