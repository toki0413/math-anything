...
class HarnessAutoGenerator:
    ...
    def generate_from_source(
        ...
        logger.info(f"   Found {len(analysis['commands'])} commands")
        logger.info(f"   Found {len(analysis['parameters'])} parameters")

        # Step 2: Semantic mapping
        logger.info("Mapping mathematical semantics...")
        ...
        logger.info("Inferring symbolic constraints...")
        ...
        logger.info(f"   Overall extraction confidence: {confidence['overall']}%")

        # Step 5: Generate code
        logger.info("Generating Harness skeleton...")
    ...
    def generate_from_docs(
        ...
        Returns:
            HarnessTemplate ready for review
        """
        logger.info(f"Analyzing documentation: {doc_path}")
        ...
        confidence = self._calculate_confidence(analysis)
        confidence["overall"] = round(confidence["overall"] * 0.8, 1)  # Doc extraction is less reliable
    ...
    def _generate_harness_code(
        ...
        if self.jinja_env:
            return self._generate_with_jinja(engine_name, engine_version, analysis, mappings, constraints, confidence)
        else:
...
''')
        ...
        logger.info(f"Generated Harness saved to: {harness_dir}")
        logger.info("   Files created:")
        logger.info(f"   - {harness_file}")
        logger.info(f"   - {init_file}")
        logger.info(f"   - {summary_file}")
        logger.info(f"   - {review_file}")
