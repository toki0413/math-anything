...
class ConstraintInference:
    ...
    def infer(
        ...
        all_context = " ".join([" ".join(ctxs) for ctxs in command_contexts.values()])
    ...
    def validate_constraint(
        ...
        """
        try:
            # Substitute variable values into the constraint expression
            eval_constraint = constraint
            for var, value in test_values.items():
                eval_constraint = eval_constraint.replace(var, str(value))

            # Use safe_eval instead of bare eval to prevent code injection
            context = {name: val for name, val in test_values.items()}
            result = safe_eval(eval_constraint, context)
