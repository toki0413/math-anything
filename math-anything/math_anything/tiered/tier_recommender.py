...
class TierRecommender:
    ...
    def recommend(self, file_analysis: FileAnalysis) -> TierRecommendation:
        ...
        upgrade_triggers = self._identify_upgrade_triggers(file_analysis, recommended_tier)
    ...
    def _calculate_complexity(self, analysis: FileAnalysis) -> ComplexityScore:
        ...
        score.total = min(
            score.system_size + score.time_scale + score.constraints + score.data_availability,
            100,
        )

        return score

    def _get_suitable_tiers(self, analysis: FileAnalysis, complexity: ComplexityScore) -> List[AnalysisTier]:
    ...
    def _get_recommended_tier(self, analysis: FileAnalysis, complexity: ComplexityScore) -> AnalysisTier:
    ...
    def _generate_reasons(self, analysis: FileAnalysis, tier: AnalysisTier) -> List[str]:
        """Generate recommendation reasons."""
        reasons = []

        if analysis.num_atoms > self.THRESHOLDS["large_system"]:
            reasons.append(f"Large system ({analysis.num_atoms} atoms), advanced methods recommended")
        elif analysis.num_atoms > self.THRESHOLDS["medium_system"]:
            reasons.append(f"Medium system ({analysis.num_atoms} atoms), topology analysis useful")

        if analysis.simulation_time > self.THRESHOLDS["long_time"]:
            reasons.append("Long simulation time, symplectic integrator needed for stability")
        ...
        if analysis.has_constraints:
            reasons.append("Constraints detected, manifold methods can reduce dimensionality")

        if analysis.has_training_data and analysis.num_atoms > self.THRESHOLDS["medium_system"]:
            reasons.append("Training data available for large system, latent space acceleration possible")

        if not reasons:
            reasons.append("Standard analysis suitable for this complexity level")

        return reasons

    def _estimate_resources(self, tier: AnalysisTier, analysis: FileAnalysis) -> ResourceRequirements:
        ...
        gpu_required = tier.value >= 4 and analysis.num_atoms > self.THRESHOLDS["medium_system"]
    ...
    def _identify_upgrade_triggers(self, analysis: FileAnalysis, current_tier: AnalysisTier) -> List[str]:
        """Identify conditions that would trigger tier upgrade."""
        triggers = []

        if current_tier.value < 4 and analysis.simulation_time > self.THRESHOLDS["medium_time"]:
            triggers.append("Long simulation time detected - consider upgrading for symplectic integrator")

        if current_tier.value < 3 and analysis.has_constraints:
            triggers.append("Constraints detected - topology analysis may provide insights")
        ...
        if (
            ...
            triggers.append("Training data available for large system - latent space acceleration possible")
