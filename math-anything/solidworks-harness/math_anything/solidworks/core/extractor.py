...
class SolidWorksExtractor:
    ...
    def extract_displacement_results(
        ...
        # Components
        ux, uy, uz = displacement_data[:, 0], displacement_data[:, 1], displacement_data[:, 2]
    ...
    def extract_thermal_results(
        ...
        result = {
            ...
            'temperature_range': float(np.max(temperature_data) - np.min(temperature_data)),
        }
    ...
    def extract_mesh_info(
        ...
        result = {
            ...
            'average_nodes_per_element': num_nodes / num_elements if num_elements > 0 else 0,
        }
        ...
        if element_quality_metrics is not None:
            result.update({
                ...
                'elements_below_threshold': int(np.sum(element_quality_metrics < 0.1)),
            })
