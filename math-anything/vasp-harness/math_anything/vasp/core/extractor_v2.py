...
from math_anything.schemas import (
    ...
    ComputationalGraph, ComputationalNode, ComputationalEdge,
    GoverningEquation, BoundaryCondition, MathematicalObject,
    TensorComponent, Discretization, Solver,
    UpdateMode, SymbolicConstraint, ParameterRelationship,
)

# Import the new parsers with symbolic constraint support
from .incar_parser import IncarParser
from .poscar_parser import PoscarParser, CrystalStructure, Lattice
from .kpoints_parser import KpointsParser, KpointsData, KpointsMode
...
class VaspSymbolicConstraints:
    ...
    @classmethod
    def validate_all_constraints(cls, incar_params: Dict[str, Any], 
                                  structure: Optional[CrystalStructure] = None,
                                  kpoints: Optional[KpointsData] = None) -> Dict[str, Any]:
    ...
    @classmethod
    def _validate_constraint(cls, constraint: SymbolicConstraint, 
...
class VaspExtractor:
    ...
    def extract(self, files: Dict[str, str], 
                options: Optional[Dict[str, Any]] = None) -> MathSchema:
        ...
        if self.incar_params:
            incar_dict = {name: param.value for name, param in self.incar_params.parameters.items()}
    ...
    def _extract_kohn_sham_equations(self) -> List[GoverningEquation]:
        ...
        equations.append(GoverningEquation(
            ...
            variables=["wavefunction", "eigenvalue", "effective_potential", "electron_density"],
            parameters={
        ...
        equations.append(GoverningEquation(
            ...
            parameters={
                "occupation": "fermi_dirac" if self.incar_params and self.incar_params.get_value("ISMEAR", 0) >= 0 else "tetrahedron",
                "smearing_width": self.incar_params.get_value("SIGMA", 0.2) if self.incar_params else 0.2,
            },
            ...
            if self.kpoints and self.kpoints.mesh:
                bcs.append(BoundaryCondition(
                    ...
                    mathematical_object=MathematicalObject(
                        ...
                        components=[
                            TensorComponent(index=[i+1], value=str(n), unit="grid")
    ...
    def _extract_computational_graph(self) -> ComputationalGraph:
        ...
        graph.add_node(ComputationalNode(
            ...
            math_semantics={
                ...
                "updates": {"target": "wavefunctions", "mode": UpdateMode.EXPLICIT_UPDATE.value},
            },
        ))
        ...
        graph.add_node(ComputationalNode(
            ...
            math_semantics={
                ...
                "updates": {"target": "density", "mode": UpdateMode.EXPLICIT_UPDATE.value},
            },
        ))
        ...
        graph.add_node(ComputationalNode(
            ...
            math_semantics={
                ...
                "convergence": {
                    "required": True,
                    "criterion": f"energy_change < {self.incar_params.get('EDIFF', 1e-4)}" if self.incar_params else "energy_change < threshold",
                    "max_iterations": self.incar_params.get_value("NELM", 60) if self.incar_params else 60,
                },
            },
        ))
    ...
    def _extract_symbolic_constraints(self) -> List[SymbolicConstraint]:
        ...
        if self.incar_params:
            for param_name, param in self.incar_params.parameters.items():
                param_constraints = VaspSymbolicConstraints.get_constraints_for_parameter(param_name)
        ...
        if self.structure:
            for constraint in VaspSymbolicConstraints.STRUCTURE_CONSTRAINTS["lattice_vectors"]:
                if "det" in constraint.expression:
                    det = np.linalg.det(self.structure.lattice.vectors)
                    satisfied = abs(det) > 1e-10
                    constraints.append(SymbolicConstraint(
                        expression=constraint.expression,
                        description=constraint.description + (" [SATISFIED]" if satisfied else " [VIOLATED]"),
                    ))
        ...
        if self.kpoints and self.kpoints.mesh:
            ...
            constraints.append(SymbolicConstraint(
                expression="n_k >= 1",
                description=f"At least one k-point required [SATISFIED: {total_k} >= 1]" if satisfied else "At least one k-point required [VIOLATED]",
            ))
