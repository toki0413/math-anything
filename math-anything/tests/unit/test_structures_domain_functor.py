...
def test_missing_functor_morphism_mapping_returns_false_not_keyerror():
    """A functor whose morphism_map omits a test morphism must return (False, str)."""
    engine, path_a, path_b = build_domain_pair_engine(
        "supervised_learning",
        {"input_dim": 2, "output_dim": 1},
        "supervised_learning",
    ...
    object_map_g = _build_cross_object_map(engine, "a", "b")
    morphism_map_g = {a_name: b_name for a_name, b_name in zip(path_a, path_b)}
    # Omit one source morphism from G's morphism_map.
    del morphism_map_g[path_a[0]]
    G = DomainFunctor(object_map_g, morphism_map_g)
