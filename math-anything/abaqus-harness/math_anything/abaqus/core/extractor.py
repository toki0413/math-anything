...
from .parser import AbaqusInputParser, Material, Step, BoundaryCondition as AbaqusBC
...
class AbaqusExtractor:
            ...
            if keyword == "*MATERIAL":
                ...
                for line in data:
                    if not line.startswith("*"):
                        parts = line.split(",")
                        if len(parts) >= 3:
                            settings.boundary_conditions.append(AbaqusBC(
    ...
    def _extract_raw_symbols(self) -> Dict[str, Any]:
        ...
        symbols = {
            ...
            "material": self.settings.material.__dict__ if self.settings and self.settings.material else {},
            "steps": [s.__dict__ for s in (self.settings.steps if self.settings else [])],
            "boundary_conditions": len(self.settings.boundary_conditions) if self.settings else 0,
