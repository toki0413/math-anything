...
from .parser import Material, Step, BoundaryCondition as AbaqusBC
...
class EnhancedAbaqusExtractor(AbaqusExtractor):
    ...
    def _generate_simulation_summary(self) -> FEMSimulationSummary:
        ...
        if self.settings:
            ...
            summary.element_types = list(set(self.settings.elements)) if self.settings.elements else []
            summary.num_steps = len(self.settings.steps)
            
            # 材料
            if self.settings.material:
                summary.material_name = getattr(self.settings.material, 'name', 'unnamed')
        ...
        if summary.total_nodes > 0:
            ...
            if summary.estimated_dof < 10000:
                summary.estimated_memory = f"约 {summary.estimated_dof * 8 / 1024:.1f} MB"
            else:
                summary.estimated_memory = f"约 {summary.estimated_dof * 8 / 1024 / 1024:.1f} GB"
    ...
    def _validate_input(self):
        ...
        if self.settings and self.settings.nlgeom:
            self.validation_warnings.append("几何非线性已启用 (nlgeom=YES)，确保材料模型兼容")
        
        # 检查单元类型
        if self.settings and self.settings.elements:
            element_types = set(self.settings.elements)
            if len(element_types) > 3:
                self.validation_warnings.append(f"使用了 {len(element_types)} 种单元类型，检查是否必要")
...
def generate_abaqus_summary_report(enhanced_result: Dict[str, Any]) -> str:
    ...
    report.append(f"  单元类型: {', '.join(summary.element_types) if summary.element_types else '未指定'}")
    report.append("")
