...
class VaspExtractor:
    ...
    def extract(
        ...
        # Parse POTCAR (if provided)
        self.potcar_data = self._parse_potcar(files.get("potcar"))
            ...
            ComputationalNode(
                ...
                math_semantics={
                    ...
                    "convergence": {
                        "required": True,
                        "criterion": (
                            f"energy_change < {self.incar_params.get_value('EDIFF', 1e-4)}"
                            if self.incar_params
    ...
    def _parse_potcar(self, potcar_path: Optional[str]) -> Dict[str, Any]:
        """Parse POTCAR to extract ENMAX values per element."""
        ...
        try:
            with open(potcar_path, "r", encoding="utf-8") as f:
                for line in f:
                    if "ENMAX" in line:
                        # Format: ENMAX  =  400.000; ENMIN  =  300.000 eV
                        parts = line.split("=")
                        if len(parts) >= 2:
                            try:
                                val = float(parts[1].split(";")[0].strip())
                                enmax_list.append(val)
                            except ValueError:
                                pass
                    elif "PAW_PBE" in line or "PAW_LDA" in line or "PAW_GGA" in line:
                        # Element line: element symbol + config
