...
class KpointsParser:
    ...
    def parse(self, filepath: str) -> KpointsData:
        ...
        # Line 2: Number of k-points (0 for automatic) or mode string
        second_line = lines[line_idx].strip()
        line_idx += 1

        # Try to parse as integer; if it fails, treat as mode string (e.g. malformed file)
        try:
            num_kpoints = int(second_line.split()[0])
        except (ValueError, IndexError):
            # Second line might be a mode string; use default of 0 and re-parse
            mode_line = second_line.lower()
            if "line" in mode_line:
                return self._parse_line_mode(lines, comment, 0, line_idx - 1)
            elif mode_line.startswith("g"):
                return self._parse_automatic(lines, comment, 0, line_idx - 1)
            else:
                # Fallback: explicit mode with unknown count
                return self._parse_explicit(lines, comment, 0, line_idx - 1)

        # Determine mode from next line
    ...
    def _parse_automatic(
        self, lines: List[str], comment: str, num_kpoints: int, line_idx: int
    ) -> KpointsData:
        ...
        mesh = KpointsMesh(subdivisions=subdivisions, shift=shift, mode=mode)

        return KpointsData(comment=comment, mode=mode, num_kpoints=0, mesh=mesh)
    ...
    def _parse_explicit(
        ...
        for i in range(num_kpoints):
            ...
            kpoint = Kpoint(coordinates=np.array(coords), weight=weight)
    ...
    def validate_sampling(
        self, kpoints: KpointsData, structure_volume: float
    ) -> List[Dict[str, Any]]:
