...
class ComputationalSettings:
    ...
    pair_coeffs: List[Dict[str, Any]] = field(default_factory=list)
    ...
    run_steps: int = 0
    minimize: Optional[Dict[str, Any]] = None
...
class LammpsInputParser:
    ...
    def _process_command(self, cmd: LammpsCommand):
        ...
        if c == "units" and args:
        ...
        elif c == "pair_coeff" and len(args) >= 2:
            self.settings.pair_coeffs.append(
                {
                    "types": args[0],
                    "params": args[1:],
                }
            ...
            try:
                self.settings.run_steps = int(args[0])
            except ValueError:
                pass

        elif c == "minimize" and len(args) >= 4:
            self.settings.minimize = {
                "etol": args[0],
                "ftol": args[1],
                "maxiter": args[2],
                "maxeval": args[3],
