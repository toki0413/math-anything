...
class LammpsExtractor(BaseEngineExtractor):
    ...
    def extract(
        ...
        if "log" in files:
            self.log_parser.parse_file(files["log"])
    ...
    # ------------------------------------------------------------------
    # 数学模型管线（覆写以添加 initial_conditions 和 parameter_relationships）
    # ------------------------------------------------------------------
    ...
    def _extract_raw_symbols(self) -> Dict[str, Any]:
        ...
        symbols = {
            "fixes": {
                f.fix_id: {"style": f.fix_style, "group": f.group_id, "params": f.args}
            ...
            "pair_style": self.settings.pair_style.style if self.settings.pair_style else "",
            "pair_style_args": self.settings.pair_style.args if self.settings.pair_style else [],
            "pair_coeffs": self.settings.pair_coeffs,
            "timestep": self.settings.timestep,
            "boundary": " ".join(self.settings.boundary_style),
            "run_steps": self.settings.run_steps,
