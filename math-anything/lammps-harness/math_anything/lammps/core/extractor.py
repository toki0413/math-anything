...
from .parser import LammpsInputParser, LammpsLogParser, FixCommand, ComputationalSettings
...
class LammpsExtractor:
    ...
    def extract(self, files: Dict[str, str], options: Optional[Dict[str, Any]] = None) -> MathSchema:
    ...
    def _extract_boundary_conditions(self) -> List[BoundaryCondition]:
        ...
        bc_map = {"p": "periodic", "f": "fixed", "s": "shrink-wrapped", "m": "shrink-wrapped-min"}
        for i, dim in enumerate(["x", "y", "z"]):
            if i < len(self.settings.boundary_style):
                bc_type = bc_map.get(self.settings.boundary_style[i], self.settings.boundary_style[i])
                bcs.append(BoundaryCondition(
                    id=f"domain_bc_{dim}",
                    type=bc_type,
                    domain={"geometric_region": f"{dim}-boundary", "entity_type": "domain"},
    ...
    def _convert_fix_deform(self, fix: FixCommand) -> Optional[BoundaryCondition]:
        ...
        if style == "erate":  # Engineering strain rate
            ...
            if dim == "x":
                components = [
                    TensorComponent(index=[1, 1], value=f"1 + {rate} * t", unit="dimensionless"),
                ...
                components = [
                    ...
                    TensorComponent(index=[2, 2], value=f"1 + {rate} * t", unit="dimensionless"),
                    TensorComponent(index=[3, 3], value="1", unit="dimensionless"),
                ]
            elif dim == "z":
                components = [
                    TensorComponent(index=[1, 1], value="1", unit="dimensionless"),
                    TensorComponent(index=[2, 2], value="1", unit="dimensionless"),
                    TensorComponent(index=[3, 3], value=f"1 + {rate} * t", unit="dimensionless"),
                ]
    ...
    def _extract_constitutive_relations(self) -> List[Dict[str, Any]]:
        ...
        if self.settings.pair_style:
            relations.append({
                ...
                "form": self._get_pair_potential_form(self.settings.pair_style.style),
    ...
    def _extract_parameter_relationships(self) -> List[ParameterRelationship]:
        ...
        if self.settings.pair_style and self.settings.pair_style.style in ("lj/cut", "lj/cut/coul/long"):
            relationships.append(ParameterRelationship(
    ...
    def _extract_numerical_method(self) -> NumericalMethod:
        ...
        if integrators:
            ...
            if integrator.fix_style == "nve":
                ...
                method.discretization.time_integrator = "velocity_verlet_with_nose_hoover"
                method.discretization.order = 2
            elif integrator.fix_style == "npt":
                method.discretization.time_integrator = "velocity_verlet_with_nose_hoover_and_barostat"
                method.discretization.order = 2
    ...
    def _fix_to_node(self, fix: FixCommand) -> Optional[ComputationalNode]:
        ...
        if fix.fix_style in ("nve", "nvt", "npt", "langevin"):
            mode = UpdateMode.IMPLICIT_LOOP if fix.fix_style in ("nvt", "npt") else UpdateMode.EXPLICIT_UPDATE
            return ComputationalNode(
                id=f"fix_{fix.fix_id}",
                type="integrator",
                math_semantics={
                    "operator_type": "symplectic_integrator" if fix.fix_style == "nve" else "thermostat_integrator",
                    "updates": {
                        "target": "phase_space",
                    ...
                    "convergence": {
                        ...
                        "criterion": "temperature_target_residual" if fix.fix_style in ("nvt", "npt") else None,
                    } if fix.fix_style in ("nvt", "npt") else {"required": False},
                },
            )
    ...
    def _extract_raw_symbols(self) -> Dict[str, Any]:
        ...
        symbols = {
            "fixes": {f.fix_id: {"style": f.fix_style, "group": f.group_id} for f in self.settings.fixes},
