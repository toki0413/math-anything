...
from .parser import FixCommand, LammpsInputParser
...
class EnhancedLammpsExtractor(LammpsExtractor):
            ...
            if style in ("lj/cut", "lj/cut/coul/long") and len(args) >= 1:
                params.append(
                    DetailedParameter(
                        ...
                        value=float(args[0]),
                        unit=(
                            "Å" if self.settings.units == "metal" else "distance units"
                        ),
                ...
                try:
                    ...
                    params.append(
                        DetailedParameter(
                            ...
                            unit=(
                                "K"
                                if self.settings.units == "metal"
                                else "temperature units"
                            ),
    ...
    def _generate_simulation_summary(self) -> SimulationSummary:
        ...
        # 1. 系统类型
        has_integrator = any(
            f.fix_style in ("nve", "nvt", "npt") for f in self.settings.fixes
        )
        has_minimize = any(
            "minimize" in str(cmd["command"]) for cmd in self.raw_commands
        )
        ...
        if self.settings.pair_style:
            ...
            summary.force_field_params = {
                "cutoff": (
                    self.settings.pair_style.args[0]
                    if self.settings.pair_style.args
                    else "default"
                )
            }
        ...
        for fix in self.settings.fixes:
            if fix.fix_style == "nvt" and len(fix.args) >= 2:
                ...
                try:
                    ...
                    summary.pressure = (
                        float(fix.args[4]) if len(fix.args) >= 5 else None
                    )
        ...
        if self.settings.boundary_style:
            ...
            for fix in self.settings.fixes:
                if fix.fix_style == "npt" and not is_periodic:
                    self.validation_warnings.append("NPT系综通常需要周期性边界条件")
...
def generate_human_readable_summary(enhanced_result: Dict[str, Any]) -> str:
    ...
    report.append("【模拟参数】")
    report.append(
        f"  时间步长: {summary.timestep} {'fs' if summary.timestep < 1 else 'ps'}"
    )
    report.append(f"  总步数: {summary.total_steps:,}")
    if summary.total_steps > 0 and summary.timestep > 0:
        total_time = summary.total_steps * summary.timestep
        report.append(
            f"  总模拟时间: {total_time} {'ps' if summary.timestep < 1 else 'time units'}"
        )
    report.append("")
