...
class TestFixDeformTensorBC(unittest.TestCase):
    ...
    def test_schema_validation(self):
        ...
        self.assertTrue(is_valid, 
            f"Schema validation failed: {validator.errors}")
    ...
    def test_fix_deform_bc_exists(self):
        ...
        self.assertGreater(len(deform_bcs), 0, 
            "No fix deform boundary condition found")
    ...
    def test_tensor_rank_2(self):
        ...
        for bc in bcs:
            if bc["id"].startswith("fix_"):
                ...
                self.assertEqual(mo.get("tensor_rank"), 2,
    ...
    def test_tensor_form_expression(self):
        ...
        for bc in bcs:
            if bc["id"].startswith("fix_"):
                mo = bc.get("mathematical_object", {})
                self.assertEqual(mo.get("tensor_form"), "F_{ij} = ∂x_i/∂X_j",
    ...
    def test_symmetry_declaration(self):
        ...
        for bc in bcs:
            if bc["id"].startswith("fix_"):
                mo = bc.get("mathematical_object", {})
                self.assertIn("symmetry", mo,
                    "Symmetry not declared")
                self.assertEqual(mo.get("symmetry"), "symmetric",
                    "Expected symmetric tensor")
    ...
    def test_trace_condition(self):
        ...
        for bc in bcs:
            if bc["id"].startswith("fix_"):
                mo = bc.get("mathematical_object", {})
                self.assertIn("trace_condition", mo,
                    "Trace condition not present")
                # Should contain det(F)
                self.assertIn("det(F)", mo.get("trace_condition", ""),
    ...
    def test_equivalent_strain_rate_formulation(self):
        ...
        for bc in bcs:
            if bc["id"].startswith("fix_"):
                ...
                self.assertGreater(len(equiv), 0,
                    "No equivalent formulations found")
                
                # Check for strain rate tensor
                strain_rate = [e for e in equiv if e.get("type") == "strain_rate_tensor"]
                self.assertEqual(len(strain_rate), 1,
                    "Expected strain rate tensor formulation")
        ...
        for bc in bcs:
            if bc["id"].startswith("fix_"):
                ...
                self.assertIn("command", impl,
                    "Software command not recorded")
                self.assertIn("style", impl,
                    "Fix style not recorded")
                self.assertIn("parameters", impl,
    ...
    def test_dual_role_declaration(self):
        ...
        for bc in bcs:
            if bc["id"].startswith("fix_"):
                ...
                self.assertTrue(dual.get("is_boundary_condition", False),
                    "Should be declared as boundary condition")
                self.assertTrue(dual.get("is_external_drive", False),
...
class TestSchemaCompleteness(unittest.TestCase):
    ...
    def test_conservation_properties(self):
        ...
        if "energy" in cp:
            self.assertIn("preserved", cp["energy"],
                "Energy preservation not specified")
    ...
    def test_raw_symbols(self):
        ...
        fixes = rs.get("fixes", {})
        deform_fixes = [k for k in fixes.keys() if "deform" in str(fixes.get(k, {}).get("style", ""))]
