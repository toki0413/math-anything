...
A Model Context Protocol server that exposes Bourbaki's mathematical
...
def _analyze_constraints(engine: str, parameters: dict, morphism_chain: list = None) -> dict:
    ...
    struct_map = {
        "vasp": lambda p: SelfConsistentProblem(operator_type="self_adjoint", variational=True),
        "lammps": lambda p: HamiltonianSystem(phase_space_dim=p.get("n_atoms", 100)*3),
        "gromacs": lambda p: HamiltonianSystem(phase_space_dim=p.get("n_atoms", 100)*3),
        "abaqus": lambda p: VariationalMinimizationProblem(),
        "ansys": lambda p: VariationalMinimizationProblem(),
        "openfoam": lambda p: NavierStokesProblem(regime="incompressible"),
    }

    struct = struct_map.get(engine.lower(), lambda p: SelfConsistentProblem())(parameters)
    return bridge.analyze_constraints(struct, parameters, morphism_chain)


def _dimensional_analyze_handler(schema: dict) -> dict:
    """Dimensional analysis via Buckingham pi."""
    from math_anything.dimensional.scaling_group import BuckinghamPiEngine, BUILTIN_QUANTITIES
    ...
    dim_check = checker.check_schema(canonical) if canonical else None

    return {
        "dimensional_check": dim_check.__dict__ if dim_check else None,
        "buckingham_pi": "Provide quantities list for pi group computation",
    }
    ...
    def __init__(self):
        self.tools = {
            ...
            "list_supported_engines": {
                "description": "List all supported simulation engines (18 total)",
                "input_schema": {
                    "type": "object",
                    "properties": {},
                },
            ...
            "analyze_constraints": {
                "description": "Analyze mathematical invariants and constraints for a calculation setup",
                "input_schema": {
                    "type": "object",
                    "properties": {
                        "engine": {
                        ...
                        "morphism_chain": {
                            "type": "array",
                            "items": {"type": "string"},
                            "description": "Optional morphism chain for propagation analysis",
                        },
                    },
                    "required": ["engine", "parameters"],
                },
                "handler": _analyze_constraints,
            },
            "dimensional_analyze": {
                "description": "Buckingham pi theorem dimensional analysis",
                "input_schema": {
                    "type": "object",
                    "properties": {
                        "schema": {"type": "object"},
                ...
                "handler": _dimensional_analyze_handler,
            },
        }
...
def main():
    parser = argparse.ArgumentParser(description="Bourbaki MCP Server — Bird is the Word")
    parser.add_argument(
        "--transport",
        choices=["stdio", "sse"],
        default="stdio",
